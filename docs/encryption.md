...
> Existe además `CryptoCore` (AES-256-GCM whole-file, formato propio `.fpkz`),
> **legacy**: solo se conserva la lectura. El cifrado al guardar es siempre ZIP estándar.
...
- **Verificación de contraseña** (`ZipExtractor.decryptZipCrypto`): el byte 11 debe
  ...
  *descriptor de datos* (bit 3) — Info-ZIP `zip` hace esto. Por eso `ArchiveEntry`
  guarda `dosTime`. ⚠️ No verificar solo contra el CRC: rompe la detección de
...
- Cabecera: método de compresión **99**, bit 0 (cifrado), **CRC = 0** (AE-2), y un
  **campo extra `0x9901`** (7 bytes): versión 2 (AE-2), vendor `"AE"`, fuerza
  (1/2/3 = 128/192/256), método real (0/8). `ArchiveEntry` expone `aesStrength` y
  `aesRealMethod`, leídos de ese campo.
...
- ⚠️ **Interop con Keka/7-Zip NO verificada por tests** (no hay 7z en el entorno);
  solo round-trip propio. **Verificación manual pendiente** (abrir en Keka).

## Flujo en la app

- **Guardar**: diálogo (`SaveOptionsSheet`) con formato + cifrado + contraseña →
  `doc.save(to:encryption:password:)` → `ZipWriter.write(..., encryption:, password:)`.
- **Abrir cifrado**: `openArchive` detecta entradas cifradas → `requiresEntryPassword`.
  ...
  recuerda (`entryPassword` + `savePassword`). `ExportPlan.zipEntry` la transporta.
...
- **Bloqueo**: `doc.isLocked` (== `requiresEntryPassword`) → solo lectura; editar
  pide la contraseña.

## Cómo probar interop

```bash
# ZipCrypto: lo hace swift test (ZipCryptoTests) contra zip/unzip del sistema.
swift test --filter ZipCryptoTests
# AES: manual en Keka — crear "Fuerte" en FilePackr y abrirlo en Keka.
```
